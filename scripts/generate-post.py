...
- 손실 위험 안내 문구 본문 내 1회 이상 포함
  예: '모든 카지노 게임은 하우스 엣지가 존재하며 손실이 발생할 수 있습니다'
...
def get_available_topics(used_ids: list) -> list:
    ...
    if not available:
        print("  🔄 50개 주제 전부 소진 → 자동 리셋 후 재시작")
        save_used_topics([])
...
6. 본문 중간에 마크다운 표(|컬럼|컬럼|) 최소 1개 포함 (RTP 비교, 게임 특징 비교 등)
...
11. [절대 금지] '무조건', '절대', '반드시' 등 단정적 행동 지시 금지
    예: 'AA는 무조건 스플릿해야 합니다' → 'AA는 스플릿을 고려할 수 있습니다'
12. [절대 금지] 손익 보장 표현 금지
    예: '손실을 최소화하는 방법', '수익을 극대화하는 전략'
13. [절대 금지] 과장된 당첨 기대 표현 금지
    예: '인생을 바꿀 수 있습니다', '대박 당첨 비법'
14. [필수] RTP 수치 언급 시 반드시 '이론적 기댓값이며 실제 결과와 다를 수 있습니다' 병기
15. [필수] 출처 명시: 'Evolution Gaming 공식 게임 수학 문서 기준' 본문 내 1회 이상 포함
16. [필수] 손실 위험 안내 문구 본문 내 1회 이상 포함
    예: '모든 카지노 게임은 하우스 엣지가 존재하며 손실이 발생할 수 있습니다'
17. [필수] eCOGRA 등 공인 감사기관 언급으로 신뢰도 강화

⚠️ 슬러그 생성 규칙:
- 아래 기존 슬러그 목록과 절대 겹치지 않게 생성
- 에볼루션카지노 관련 키워드 포함 권장
- 제목에 숫자(N가지)가 있으면 반드시 슬러그에 숫자 포함
- 한글 제목을 전체 영문으로 번역하여 슬러그 생성 (일부분만 번역 금지)
