...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://wooriwin.com/blog/${slug}`,
    },
    robots: {
      index: true,
      follow: true,
      ...
      <main className="min-h-screen bg-gray-900 text-white">
            ...
            <div className="text-gray-300 space-y-5 leading-relaxed text-base md:text-lg">
              <ReactMarkdown
                ...
                components={{
                  ...
                  th: (props) => (
                    <th
                      className="border border-gray-600 px-4 py-2 text-yellow-400 text-left"
                      {...props}
                    />
                  ),
                  td: (props) => (
                ...
                <div className="space-y-4">
                  {post.faq.map(
                    (
                      ...
                      <details
                        key={i}
                        className="bg-gray-800 rounded-xl p-5 group cursor-pointer"
                      >
                        <summary className="font-semibold text-white flex justify-between items-center list-none">
                          {f.q}

                          <span className="text-yellow-400 text-xl transition-transform group-open:rotate-45">
                            +
                          </span>
                        </summary>
                    ...
                    {post.relatedPosts.map(
                      (r: {
                        ...
                        <li key={r.slug}>
                          <Link
                            href={`/blog/${r.slug}`}
                            className="text-gray-400 text-sm hover:text-yellow-400 transition leading-snug block"
                          >
                            → {r.title}
                          </Link>
                        </li>
        ...
        <footer className="bg-gray-950 py-10 px-4 text-center">
          ...
          <p className="text-gray-600 text-xs">
            © 2026 WOORIWIN. All rights reserved.
          </p>
        </footer>
      </main>
    </>
